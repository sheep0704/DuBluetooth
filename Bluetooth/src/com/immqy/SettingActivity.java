...
import com.immqy.widget.SwitchButton;
...
public class SettingActivity extends Activity {
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        if (resultCode == RESULT_OK) {
            uri = data.getData();
            try {
                Bitmap bitmap = BluetoothUtils.getBitmap(uri, SettingActivity.this);
                imageView.setImageBitmap(bitmap);
                String path = BluetoothUtils.getPath(uri, SettingActivity.this);
                provider.save("image", path);
                key = BluetoothUtils.createKey(path);
